...
class ProductAttributeDefinitionExporter(Component):
    ...
    def _update_attribute_with_result(self, result):
        ...
        _logger.info("We got the result with options: %s", result['options'])
        for option in result['options']:
            if not option['value']:
                continue
            # First try - direct match on code
            option_binding = self.binding.magento_attribute_value_ids.filtered(lambda b: b.code == option['value'])
            # Second try - match on label - with no code set already
            if not option_binding:
                option_binding = self.binding.magento_attribute_value_ids.filtered(lambda b: b.name == option['label'] and not b.code)
