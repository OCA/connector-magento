...
class MagentoStoreview(models.Model):
    ...
    base_media_url = fields.Char(
        help=(
            "Base URL to retrieve product images. Used for Magento2 only. "
    ...
    def import_sale_orders(self):
        ...
        for storeview in self:
            ...
            to_date = import_start_time - timedelta(minutes=backend.order_import_delay)
...
class StoreviewAdapter(Component):
    ...
    def read(self, external_id, attributes=None):
        ...
        if self.collection.version == "2.0":
            ...
            storeview = next(
                record
                for record in self._call("store/storeViews")
                if record["id"] == external_id
            )
            storeview.update(
                next(
                    record
                    for record in self._call("store/storeConfigs")
                    if record["id"] == external_id
                )
            )
            return storeview
