...
class MagentoProductProduct(models.Model):
    ...
    website_ids = fields.Many2many(
        comodel_name="magento.website", string="Websites", readonly=True
    )
    ...
    auto_content_sync = fields.Boolean(required=True, default=True)
...
class ProductProduct(models.Model):
    ...
    @api.multi
    def action_publish_to_magento(self, backend_ids):
        # Create the records per backend
        if not backend_ids:
            backend_ids = self.env["magento.backend"].sudo().search([])
        to_create = []

        for record in self:
            for backend in backend_ids:

                # Find existing backend bindings
                existing = (
                    self.env["magento.product.product"]
                    .sudo()
                    .search(
                        [("backend_id", "=", backend.id), ("odoo_id", "=", record.id),]
                    )
                )
        ...
        created = self.env["magento.product.product"].sudo().create(to_create)
        created.recompute_magento_qty()
...
class ProductProductAdapter(Component):
    ...
    def write(self, external_id, data, storeview_id=None):
        ...
        return self._call(
            "products/%s" % (self.escape(external_id)), data, http_method="put"
        )
    ...
    def create(self, sku, data):
        ...
        return self._call("products", data, http_method="post")
...
class ProductProductListener(Component):
    ...
    def on_record_write(self, record, fields=None):
        ...
        for binding in record.with_context(active_test=False).magento_bind_ids.filtered(
            lambda p: p.auto_content_sync
        ):
            binding.with_delay().export_inventory(fields)
...
class MagentoBindingProductListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=20).export_inventory(fields)
