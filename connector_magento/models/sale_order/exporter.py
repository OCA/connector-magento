...
class StateExporter(Component):
    ...
    def run(self, binding, allowed_states=None, comment=None, notify=False):
        ...
        if (record.get("status") or record["state"]) == magento_state:
            return _("Magento sales order is already " "in state %s") % magento_state
        if self.collection.version == "2.0":
            ...
            self.backend_adapter.add_comment(
                external_id, magento_state, comment=comment, notify=notify
            )
