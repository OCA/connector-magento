...
class SaleImportRule(Component):
    ...
    def _rule_paid(self, record, method):
        ...
        if (
            not record.get('payment', {}).get('amount_paid') and
            not record.get('payment', {}).get('base_amount_paid') and
            not record.get('payment', {}).get('base_amount_paid_online') and
            not record.get('payment', {}).get('total_paid')
        ):
        ...
        for line in record.get('items', []):
            ...
            if self.collection.version == '1.7':
                self._import_dependency(
                    line['product_id'], 'magento.product.product')
            else:
                self._custom_import_dependency('magento_id',
                                               line['product_id'], line['sku'],
        ...
        if self.collection.version == '1.7':
            ...
            if field in record:
                product = binder.custom_to_internal(
                    'magento_id', record[field], unwrap=True)
            if not product:
                field = 'sku'
                product_bind = binder.to_internal(record[field])
                if product_bind:
