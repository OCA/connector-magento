...
class MagentoHelper:
    ...
    def get_next_id(self):
        self.cr.execute(
            "SELECT max(external_id::int) FROM %s ", (AsIs(self.model._table),)
        )
...
class MagentoTestCase(SavepointComponentCase):
    ...
    def setUpClass(cls):
        ...
        cls.workflow = cls.env.ref("sale_automatic_workflow.manual_validation")
    ...
    @classmethod
    def create_binding_no_export(cls, model_name, odoo_id, external_id=None, **cols):
        ...
        if cols:
            values.update(cols)
        return cls.env[model_name].with_context(connector_no_export=True).create(values)
    ...
    def _import_record(cls, model_name, magento_id, cassette=True):
        ...
        def run_import():
            with mute_logger(
                ...
                if cls.backend.version != "1.7":
                    return cls.env[model_name].import_record(cls.backend, magento_id)
                with mock_urlopen_image():
                    cls.env[model_name].import_record(cls.backend, magento_id)
        ...
        binding = cls.env[model_name].search(
            [("backend_id", "=", cls.backend.id), ("external_id", "=", str(magento_id))]
        )
