...
class TestImportProduct(Magento2SyncTestCase):
    ...
    def test_import_product(self):
        ...
        self.env["magento.product.product"].import_record(self.backend, "24-WG02")

        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "24-WG02")]
        )
    ...
    def test_import_product_configurable(self):
        ...
        self.env["magento.product.product"].import_record(self.backend, "MH01")

        product_model = self.env["magento.product.product"]
        products = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "MH01")]
        )
    ...
    def test_import_product_virtual(self):
        ...
        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "TRN")]
        )
    ...
    def test_import_product_quoted_sku(self):
        ...
        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "bla/koe")]
        )
