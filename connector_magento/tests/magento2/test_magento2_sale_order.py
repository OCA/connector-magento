...
ExpectedOrderLine = namedtuple(
    "ExpectedOrderLine", "product_id name price_unit product_uom_qty"
)
...
class TestSaleOrder(Magento2SyncTestCase):
    ...
    def _import_sale_order(self, increment_id, cassette=True):
        return self._import_record(
            "magento.sale.order", increment_id, cassette=cassette
        )
    ...
    def test_import_sale_order_with_configurable(self):
        ...
        prod1 = self.env["magento.product.product"].search(
            [("external_id", "=", "MH09-XS-Blue"), ("backend_id", "=", self.backend.id)]
        )
        prod2 = self.env["magento.product.product"].search(
            [("external_id", "=", "RACER"), ("backend_id", "=", self.backend.id)]
        )
    ...
    def test_cancel_export(self):
        ...
        with recorder.use_cassette("test_sale_order_cancel_export") as cassette:
            ...
            self.assertEqual(
                cassette.requests[0].uri, "http://magento/index.php/rest/V1/orders/12"
            )
            self.assertEqual(
                cassette.requests[1].uri, "http://magento/index.php/rest/V1/orders"
            )
            self.assertEqual(
    ...
    def test_copy_quotation_export_state(self):
        ...
        self.assertEqual(
            cassette.requests[3].uri, "http://magento/index.php/rest/V1/orders/12"
        )
        self.assertEqual(
            cassette.requests[4].uri, "http://magento/index.php/rest/V1/orders"
        )
        self.assertEqual(
