...
class TestImportPartner(MagentoSyncTestCase):
    def setUp(self):
        ...
        self.create_binding_no_export(
            "magento.res.partner.category", existing_category, 1
        )
    ...
    def test_import_partner_no_address(self):
        ...
        partner = self.model.search(
            [("external_id", "=", "139"), ("backend_id", "=", self.backend.id)]
        )
    ...
    def test_import_partner_individual_1_address(self):
        ...
        partner = self.model.search(
            [("external_id", "=", "136"), ("backend_id", "=", self.backend.id)]
        )
        self.assertEqual(len(partner), 1)
        ...
        self.assertEqual(
            address_bind.external_id,
            "92",
            msg="The merged address should be the " "billing address",
        )
        self.assertEqual(partner.company_id.id, self.backend.warehouse_id.company_id.id)
    ...
    def test_import_partner_individual_2_addresses(self):
        ...
        partner = self.model.search(
            [("external_id", "=", "65"), ("backend_id", "=", self.backend.id)]
        )
        self.assertEqual(len(partner), 1)
        # Name of the billing address
    ...
    def test_import_partner_company_1_address(self):
        ...
        partner = self.model.search(
            [("external_id", "=", "104"), ("backend_id", "=", self.backend.id)]
        )
        self.assertEqual(len(partner), 1)
        ...
        self.assertEqual(
            partner.child_ids[0].type,
            "invoice",
            msg="The billing address should be of " "type 'invoice'",
        )
    ...
    def test_import_partner_company_2_addresses(self):
        ...
        partner = self.model.search(
            [("external_id", "=", "99"), ("backend_id", "=", self.backend.id)]
        )
        self.assertEqual(len(partner), 1)
        # Company of the billing address
