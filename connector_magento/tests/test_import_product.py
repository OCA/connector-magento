...
from .common import MagentoSyncTestCase, mock_urlopen_image, recorder
...
class TestImportProduct(MagentoSyncTestCase):
    ...
    def _create_category(self, name, external_id):
        ...
        self.create_binding_no_export("magento.product.category", category, external_id)
    ...
    def test_import_product(self):
        ...
        with mock_urlopen_image():
            ...
            self.env["magento.product.product"].import_record(self.backend, "337")

        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "337")]
        )
    ...
    def test_import_product_category_missing(self):
        ...
        self.assertEqual(
            len(
                self.env["magento.product.category"].search(
        ...
        with mock_urlopen_image():
            self.env["magento.product.product"].import_record(self.backend, "382")

        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "382")]
        )
        self.assertEqual(len(product), 1)
    ...
    def test_import_product_configurable(self):
        ...
        self.env["magento.product.product"].import_record(self.backend, "408")

        product_model = self.env["magento.product.product"]
        products = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "408")]
        )
    ...
    def test_import_product_bundle(self):
        ...
        with self.assertRaises(InvalidDataError):
            self.env["magento.product.product"].import_record(self.backend, "447")

    @recorder.use_cassette
    def test_import_product_grouped(self):
        """ Grouped should fail: not yet supported """
        with self.assertRaises(InvalidDataError):
            self.env["magento.product.product"].import_record(self.backend, "555")
    ...
    def test_import_product_virtual(self):
        ...
        self.env["magento.product.product"].import_record(self.backend, "563")

        product_model = self.env["magento.product.product"]
        product = product_model.search(
            [("backend_id", "=", backend_id), ("external_id", "=", "563")]
        )
