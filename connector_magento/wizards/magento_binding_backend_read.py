...
class MagentoBindingBackendRead(models.TransientModel):
    ...
    def _get_translated_model_name(self, model_name):
        ...
        search_result = self.env["ir.model"].name_search(model_name, operator="=")
        if search_result:
    ...
    def _default_magento_backend_id(self):
        ...
        if active_model and active_model != "magento.backend":
            raise UserError(
                _("The wizard must be launched from a magento " "backend model")
            )
        active_id = self._context.get("active_ids", [])
        if len(active_id) > 1:
            raise UserError(
                _("The wizard must be launched on a single " "magento backend instance")
            )
        active_id = (active_id and active_id[0]) or self._context.get("active_id")
    ...
    def _get_magento_binding_model(self):
        ...
        for component_class in component_classes:
            ...
            for model_name in apply_on:
                ret.append((model_name, self._get_translated_model_name(model_name)))
        return ret
    ...
    def action_get_info(self):
        ...
        with self.magento_backend_id.work_on(self.magento_binding_model) as work:
