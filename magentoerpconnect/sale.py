...
class sale_order_line(Model):
    ...
    _columns = {
        ...
        # NEEDS REVIEW : Do we still need this !?
    ...
    def invoice_line_create(self, cr, uid, ids, context):
        """ In order to have a one2many link between the sale order line and the various
        invoice line, we overwrite this method. 
        We made that cause there is only a many2many link between them by default. We were not
        able to retrieve the sale order line ID on magento side from an invoice line.
        ...
        for line in self.browse(cr, uid, ids, context):
            created_line_id = super(sale_order_line, self).invoice_line_create(cr, uid, [line.id], context)
            # Test if magento_sale_order_line exists, if yes create a magento_invoice_line
    ...
    _columns = {
        ...
        _sql_constraints = [
            ('magento_uniq', 'unique(backend_id, magento_id)',
             'A sale order line with same ID on Magento already exists.'),
        ]
