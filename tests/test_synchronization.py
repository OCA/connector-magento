...
class TestImportMagento(SetUpMagentoSynchronized):
    ...
    def test_36_import_sale_order_with_bundle(self):
        ...
        mag_order_ids = mag_order_model.search(self.cr, self.uid, [
            ('backend_id', '=', backend_id),
        ...
        mag_order = mag_order_model.browse(self.cr,
                                           ...
                                           mag_order_ids[0])
        match_line = [
            {'mag_id': '164', 'price': 5311.73, 'nb_children': 5,
             'parent_item_id': False},
            {'mag_id': '139', 'price': 0.00, 'nb_children': 0,
             'parent_item_id': '218'},
            {'mag_id': '153', 'price': 0.00, 'nb_children': 0,
             'parent_item_id': '218'},
            {'mag_id': '143', 'price': 0.00, 'nb_children': 0,
             'parent_item_id': '218'},
            {'mag_id': '154', 'price': 0.00, 'nb_children': 0,
             'parent_item_id': '218'},
            {'mag_id': '160', 'price': 0.00, 'nb_children': 0,
             'parent_item_id': '218'},
            {'mag_id': '166', 'price': 750.00, 'nb_children': 0,
             'parent_item_id': False},
            {'mag_id': None, 'price': 32.33, 'nb_children': 0,
             'parent_item_id': False},
            {'mag_id': '138', 'price': 103.93, 'nb_children': 0,
             'parent_item_id': '212'},
            {'mag_id': '148', 'price': 68.58, 'nb_children': 0,
             'parent_item_id': '212'},
            {'mag_id': '150', 'price': 207.16, 'nb_children': 0,
             'parent_item_id': '212'},
            {'mag_id': '141', 'price': 104.61, 'nb_children': 0,
             'parent_item_id': '212'},
            {'mag_id': '152', 'price': 484.98, 'nb_children': 0,
             'parent_item_id': '212'},
        ]

        product_env = get_environment(self.session, 'magento.product.product',
                                      backend_id)
        product_binder = product_env.get_connector_unit(Binder)
        line_env = get_environment(self.session, 'magento.sale.order.line',
                                   backend_id)
        line_binder = line_env.get_connector_unit(Binder)
        for index, line in enumerate(mag_order.order_line):
            ref_line = match_line[index]
            mag_product_id = product_binder.to_backend(line.product_id.id,
                                                       wrap=True)
            ...
            if line.line_parent_id:
                mag_parent_line_id = line_binder.to_backend(
                    line.line_parent_id.id, wrap=True)
            else:
